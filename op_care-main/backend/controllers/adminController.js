...
exports.getDoctorAvailability = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(doctorId).select('availability fullName email');
...
exports.updateDoctorAvailability = async (req, res, next) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(doctorId, { $set: update }, { new: true }).select('availability');
    if (!user) return res.status(404).json({ error: 'Doctor not found' });
    res.json(user.availability || {});
  } catch (err) {
    next(err);
...
exports.getAllOperations = async (req, res, next) => {
  try {
    const ops = await Operation.find().populate('doctor', 'fullName email').populate('patient', 'fullName email');
    res.json(ops);
  } catch (err) { next(err); }
};
...
exports.createOperation = async (req, res, next) => {
  try {
    const { doctor, patient, date, type, outcome, notes } = req.body;
    if (!doctor || !patient || !date || !type) return res.status(400).json({ error: 'doctor, patient, date, and type are required' });
    const op = await Operation.create({ doctor, patient, date, type, outcome, notes });
...
exports.updateOperationAdmin = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { doctor, patient, date, type, outcome, notes } = req.body;
    const op = await Operation.findByIdAndUpdate(id, { doctor, patient, date, type, outcome, notes }, { new: true });
...
exports.createNotification = async (req, res, next) => {
  try {
    ...
    const note = await Notification.create({ message, user: user || null, roleTarget: roleTarget || 'all' });
...
exports.getVideoUsageStats = async (req, res, next) => {
  try {
    const totalVideo = await Booking.countDocuments({ consultationType: 'video' });
    const perDoctor = await Booking.aggregate([
      ...
      { $project: { doctorId: '$_id', count: 1, doctorName: '$doctor.fullName', doctorEmail: '$doctor.email' } }
    ]);
